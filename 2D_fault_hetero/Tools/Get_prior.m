...
for ifield=1:Pr.n_fields
    ...
    for i=1:N_En
        ...
        pri.len{1}=mean(Pr.K(ifield).per.len(1).lim)*ones(Grid.N,1);
        pri.len{2}=mean(Pr.K(ifield).per.len(2).lim)*ones(Grid.N,1);
        fields_per(1+(ifield-1)*Grid.N:Grid.N+(ifield-1)*Grid.N,i)=randn(Grid.N,1);
        for idim=1:dim
            Y=exp(Pr.K(ifield).per.len(idim).lim(1))+(exp(Pr.K(ifield).per.len(idim).lim(2))-exp(Pr.K(ifield).per.len(idim).lim(1)))*X(i,idim);
            fields_le(idim+(ifield-1)*dim,i)=log(Y);
        end
    end
