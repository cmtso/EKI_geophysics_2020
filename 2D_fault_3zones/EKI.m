...
option=1; %option=1 for variable lengthscale and option=0 for constant lengthscale
...
for i = 1:size(sigma,1)
    zone1_prob(i) = sum(sigma(i,:)== 0.01) ;
    var_log_res(i) = var(log10(1./sigma(i,:))) ./ log10(1./sigma_mean(i));
end
...
add_list = {"Log mean resistivity","Zone 1 probability","CV(log res)"} ;
add_list = {"Log mean resistivity"};
...
%caxis([0 0.025])
hold on, plot([0 24 24 48],[-7 -7 -5 -5],'r-','LineWidth',2), hold off
rectangle('Position',[0 -10 48 10])
title('Mean est. resistivities are 242, 798 and 2372 Ohm m')
